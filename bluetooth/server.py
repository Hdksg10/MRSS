...
server_socket=None
#WIFI配置类
wlprofle = wlan.wlanprofile("empty", "empty") 
...
def serveSocket(sock,info):
    ...
    while True:
        ...
        #接受客户端传入的ssid
        if(receive == "ssid="):
            ssid=sock.recv(1024).decode('utf-8')
            print(ssid)
            sock.send(ssid.encode('utf8'))            
            wlprofle.getssid(ssid)
        #接受客户端传入的pwk
        elif(receive == "pwk="):
            pwk=sock.recv(1024).decode('utf-8')
            print(pwk)
            sock.send(pwk.encode('utf8'))
            wlprofle.getpwk(pwk)
            #测试是否连接成功
            isConnceted = wlprofle.setwlan()
            #连接成功的回传信息
            if(isConnceted):
                sock.send("internet connected".encode('utf8'))
            else:
                sock.send("cannot connect to internet".encode('utf8'))
        else:
        #打印刚刚读到的东西(info=地址)
